...
public class IndicatorResourceIntTest {
    ...
    @Test
    @Transactional
    public void getAllIndicatorsByMetaDadoIsEqualToSomething() throws Exception {
        // Initialize the database
        MetaDado metaDado = MetaDadoResourceIntTest.createEntity(em);
        em.persist(metaDado);
        em.flush();
        indicator.addMetaDado(metaDado);
        indicatorRepository.saveAndFlush(indicator);
        Long metaDadoId = metaDado.getId();

        // Get all the indicatorList where metaDado equals to metaDadoId
        defaultIndicatorShouldBeFound("metaDadoId.equals=" + metaDadoId);

        // Get all the indicatorList where metaDado equals to metaDadoId + 1
        defaultIndicatorShouldNotBeFound("metaDadoId.equals=" + (metaDadoId + 1));
    }
    ...
    @Test
    @Transactional
    public void getAllIndicatorsByGroupIsEqualToSomething() throws Exception {
        // Initialize the database
        GrupIndicator group = GrupIndicatorResourceIntTest.createEntity(em);
        em.persist(group);
        em.flush();
        ...
        // Get all the indicatorList where group equals to groupId
        defaultIndicatorShouldBeFound("groupId.equals=" + groupId);

        // Get all the indicatorList where group equals to groupId + 1
        defaultIndicatorShouldNotBeFound("groupId.equals=" + (groupId + 1));
    }


    @Test
    @Transactional
    public void getAllIndicatorsByYearIsEqualToSomething() throws Exception {
        // Initialize the database
        ...
        // Get all the indicatorList where year equals to yearId
        defaultIndicatorShouldBeFound("yearId.equals=" + yearId);

        // Get all the indicatorList where year equals to yearId + 1
        defaultIndicatorShouldNotBeFound("yearId.equals=" + (yearId + 1));
    }
