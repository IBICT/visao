...
import br.com.ibict.repository.LayerRepository;
import br.com.ibict.service.LayerService;
import br.com.ibict.web.rest.errors.ExceptionTranslator;
import br.com.ibict.service.dto.LayerCriteria;
import br.com.ibict.service.LayerQueryService;
...
import br.com.ibict.domain.enumeration.TypeLayer;
/**
...
public class LayerResourceIntTest {
    ...
    @Test
    @Transactional
    public void getAllLayersByMetaDadoIsEqualToSomething() throws Exception {
        // Initialize the database
        MetaDado metaDado = MetaDadoResourceIntTest.createEntity(em);
        em.persist(metaDado);
        em.flush();
        layer.addMetaDado(metaDado);
        layerRepository.saveAndFlush(layer);
        Long metaDadoId = metaDado.getId();

        // Get all the layerList where metaDado equals to metaDadoId
        defaultLayerShouldBeFound("metaDadoId.equals=" + metaDadoId);

        // Get all the layerList where metaDado equals to metaDadoId + 1
        defaultLayerShouldNotBeFound("metaDadoId.equals=" + (metaDadoId + 1));
    }
