...
public class GroupLayerResourceIntTest {
    ...
    @Test
    @Transactional
    public void getAllGroupLayersByUserIsEqualToSomething() throws Exception {
        // Initialize the database
        User user = UserResourceIntTest.createEntity(em);
        em.persist(user);
        em.flush();
        groupLayer.setUser(user);
        groupLayerRepository.saveAndFlush(groupLayer);
        Long userId = user.getId();

        // Get all the groupLayerList where user equals to userId
        defaultGroupLayerShouldBeFound("userId.equals=" + userId);
