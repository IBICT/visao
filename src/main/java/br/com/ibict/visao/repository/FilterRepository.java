...
public interface FilterRepository extends JpaRepository<Filter, Long>, JpaSpecificationExecutor<Filter> {
    ...
    @Query(value = "select distinct filter from Filter filter left join fetch filter.regions left join fetch filter.categories",
    ...
    @Query("select filter from Filter filter left join fetch filter.regions left join fetch filter.categories where filter.id =:id")
