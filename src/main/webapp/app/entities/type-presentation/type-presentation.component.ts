...
export class TypePresentationComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        this.typePresentationService
            ...
            .subscribe(
                (res: HttpResponse<ITypePresentation[]>) => this.paginateTypePresentations(res.body, res.headers),
    ...
    registerChangeInTypePresentations() {
        this.eventSubscriber = this.eventManager.subscribe('typePresentationListModification', response => this.reset());
    }
    ...
    private paginateTypePresentations(data: ITypePresentation[], headers: HttpHeaders) {
        this.links = this.parseLinks.parse(headers.get('link'));
        this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
        for (let i = 0; i < data.length; i++) {
            this.typePresentations.push(data[i]);
        }
    }
